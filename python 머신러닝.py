...
---------- 머신러닝 혼합 아이디어  ----------
 ...
 - 산술평균, 기하평균, 멱평균
 - 가중치평균
 - 낮은 성능부터 차례대로 평균해가기(ex 4개로 기하평균 할 경우 낮은 성능부터 2개씩 기하평균해서 3번을 걸쳐서 할 경우 성능 향상하였음)
 - low top 가중치 평균 (ex 4개 기하평균할 경우 낮은 성능부터 2개씩 기하평균해서 파일하나 만들고,
                    또 반대로 높은 성능부터 2개씩 기하평균해서 파일하나 만들음. 여기서 보편적으로 낮은 성능부터 앙상블 한 것이 성능이 훨 좋음,
                    이때 low파일에 대한 성능을 한번더 가중치 조절해주기 위해서 top과 함께 가중치 앙상블을 하는 것임.
                    실제로 성능향상의 효과를 본 방법은 low * 0.95 + top * 0.05 였음)
